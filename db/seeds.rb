...
CSV.foreach(Rails.root.join('test_data/tbl_db_list.txt'), encoding: "ISO-8859-1", headers: true) do |row|
  next if row[:jhu_id] == 'jhu_id'
  Database.new(vendor_id: row[:vendor_id], name: row[:db_name], url: row[:url],
    enable_proxy: row[:enable_proxy], description: row[:description],
    internal_note: row[:internal_note], jhu_id: row[:jhu_id]).save!
end